...
set :deploy_to, "/usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}"
set :site_root, "/usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}/#{current_dir}"
set :keep_releases, 3
...
namespace :deploy do
  ...
  task :restart, :roles => :app do
    run "mkdir -p /usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}/#{current_dir}/tmp"
    run "sudo chmod 775 -R /usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}/#{current_dir}/tmp"
    run "sudo chmod 775 -R /usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}/#{current_dir}/public/assets"
    run "touch #{File.join("/usr/share/nginx/ruby/apps/#{APP_NAME_SPACE}/#{current_dir}/tmp",'restart.txt')}"
  end
end